...
```{r}
host_plant %<>% gather("species", "scientificName", starts_with("species"), na.rm = FALSE)
```
...
Add a column `relation` to `raw_data` and `host_plant` to indicate whether the species in `scientificName`is a parasite or a host:

```{r}
raw_data %<>% mutate(relation = "parasite")
host_plant %<>% mutate(relation = "host")
```
